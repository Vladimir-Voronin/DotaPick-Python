...
async function getCurrentHeroListFromDB() {
    ...
    const promiseWithHeroList = heroListJsonRequest().then((heroes) => {
        ...
        for (let heroJson of heroes) {
            ...
            hero = new Hero(
                heroJson.dotabuff_name,
                heroJson.name,
                heroJson.general_winrate,
                heroJson.winrate_dict,
                heroJson.image_path,
                new Set(heroJson.roles_set),
                true,
                (alliesSet = new Set(heroJson.allies_set)),
            );
            heroList.push(hero);
        }
        return heroList;
