...
function customRenderNumber50Average(data) {
    ...
    if (data >= 50) {
        return `<div class="positive-number">${data.toFixed(
            precision,
        )}${postfix}</div>`;
    } else {
        return `<div class="negative-number">${data.toFixed(
            precision,
        )}${postfix}</div>`;
    }
}
        ...
        columns: [
            {
                ...
                render: function (data) {
                    return `<img data-value=${
                        data.dotabuffName
                    } class="image-full-block"  src=${
                        defaultHeroImagesPath +
    ...
    for (const hero of mainObjects.recommendationList.heroList) {
        if (hero.visibility) {
            newRow = {
                ...
                'hero-extra-points':
                    hero.teamCorrectionWinrate + hero.additionalWinrate,
            };
...
function makeRecalculationAndUpdateTable() {
    mainObjects.recommendationList.fullRecalculation(
